...
RAW_START="$(curl -s "$API_BASE/lifecycle/canary/start")"
emit "$(printf '%s' "$RAW_START" | $FMT --type canary-start 2>&1)"
show_cmd "curl -s \$API_BASE/lifecycle/canary/watch | python3 scripts/fmt.py --type canary-watch"
RAW_WATCH="$(curl -s "$API_BASE/lifecycle/canary/watch")"
emit "$(printf '%s' "$RAW_WATCH" | $FMT --type canary-watch 2>&1)"
A_OK=1; B_OK=1
...
RAW_PROMOTE="$(curl -s "$API_BASE/lifecycle/canary/promote")"
emit "$(printf '%s' "$RAW_PROMOTE" | $FMT --type canary-promote 2>&1)"
show_cmd "curl -s \$API_BASE/lifecycle/canary/rollback | python3 scripts/fmt.py --type canary-rollback"
RAW_ROLLBACK="$(curl -s "$API_BASE/lifecycle/canary/rollback")"
emit "$(printf '%s' "$RAW_ROLLBACK" | $FMT --type canary-rollback 2>&1)"
A_OK=1; B_OK=1
